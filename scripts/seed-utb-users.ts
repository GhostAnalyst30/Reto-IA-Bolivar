...
const USERS: { email: string; role: string; full_name: string; faculty_id?: string; password?: string }[] = [
  { email: 'admin@utb.demo', role: 'admin', full_name: 'Admin UTB' },
  { email: 'rector@utb.demo', role: 'rector', full_name: 'Rector UTB' },
  { email: 'vicerrector@utb.demo', role: 'vice_president', full_name: 'Vicerrector UTB' },
  { email: 'decano@utb.demo', role: 'dean', full_name: 'Decano UTB', faculty_id: ENGINEERING_FACULTY },
  { email: 'director.programa@utb.demo', role: 'area_head', full_name: 'Director de Programa UTB', faculty_id: ENGINEERING_FACULTY },
  ...Array.from({ length: 10 }, (_, i) => ({
    email: `estudiante${String(i + 1).padStart(2, '0')}@utb.demo`,
    role: 'student',
